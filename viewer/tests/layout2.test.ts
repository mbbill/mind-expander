...
import type { TypeBox } from '../src/analysis/layout_bak.ts';
import {
  BAND_GRID_CELL_H,
  BAND_GRID_CELL_W,
  FIELD_ROW_H,
...
import type { Obstacle, PositionedType } from '../src/layout2/types.ts';
...
describe('buildLayoutV2 — Layout shape', () => {
  it('returns populated modules + types for the small fixture (all expanded)', () => {
    const layout = buildLayoutV2(smallFixtureInputs(SMALL_EXPANDED));
  ...
  it('expanded type with fields renders a TypeBox of the expected height', () => {
    ...
    const layout = buildLayoutV2(smallFixtureInputs([...SMALL_EXPANDED, 'c::App']));
  ...
  it('carries stable header hit geometry for the renderer', () => {
    ...
    const collapsed = buildLayoutV2(buildInputs(c, [], ['c', 'c::m']));
    const expanded = buildLayoutV2(buildInputs(c, [], ['c', 'c::m', 'c::m::Thing']));
  ...
  it('keeps fallback-pressure fixture geometry while block layout isolation is active', () => {
    ...
    const firstGeometry = computeGeometry(inputs);
    const layout = buildLayoutV2({ ...inputs, measureText: measure });
